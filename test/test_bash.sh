...
elif ! grep -q '^STEP1_PROFILE version=3 backend=cpu mode=loocv ' ${REGENIE_PATH}test/fit_bin_out.log; then
  print_custom_err "Step 1 profiling header is missing."
elif [ "`grep -c '^STEP1_PROFILE scope=' ${REGENIE_PATH}test/fit_bin_out.log`" != "2" ]; then
