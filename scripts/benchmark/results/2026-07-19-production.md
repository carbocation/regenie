...
| Binary | 0-10% | 500,000 | 700,000 | 32 | **36.53 min / $2.24** | 1,065.89-1,161.49 min / $13.80-$15.04 | **29.18-31.80x** | Current full run measured; upstream range projected from a matched 210-block Level 0 prefix |
| Survival | 0-10% | 500,000 | 700,000 | 32 | **33.36 min / $2.04** | 540.91-586.64 min / $7.00-$7.60 | **16.21-17.59x** | Current full run measured; upstream range projected from a matched 192-block Level 0 prefix |
| Quantitative | none | 500,000 | 700,000 | 1 | **1.84 min / $0.11** | 136.79 min / $1.77 | **74.38x** | Both full runs measured directly |
...
The slow upstream P=32 jobs were stopped after their Level 0 rates stabilized.
The lower bound extrapolates the timed read, residualization, working-matrix,
and ridge phases; the upper bound extrapolates observed wall time, which also
captures uninstrumented file work. The binary prefix covered 208,742 variants
in 210 completed blocks, and the survival prefix covered 191,046 variants in
192 completed blocks. Level 1 uses the measured upstream P=8 trait-fit ranges
scaled to the 32-trait panels; upstream fits traits serially.
...
| Binary, 0-10% missing | 500,000 | 700,000 | 32 | 15.55 s | 976.18 s | 1,145.98 s | 54.04 s | **2,191.74 s** | 14.3% / 85.4% |
| Survival, 0-10% missing | 500,000 | 700,000 | 32 | 20.75 s | 1,020.52 s | 905.00 s | 55.38 s | **2,001.65 s** | 12.4% / 71.0% |
...
Binary and survival Level 1 contain much more GPU work. Their accelerated
weighted Gram kernels reach 100% GPU utilization; mean Level 1 utilization is
85.4% for binary and 71.0% for survival. Raising utilization in the I/O phases
would not by itself reduce wall time.
...
accounts for 401.05 seconds of the 1,019.98-second Level 0 phase in the
production-size run.
...
outputs are compared with validated controls before a result is promoted to a
headline row.
