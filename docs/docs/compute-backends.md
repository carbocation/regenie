...
| `REGENIE_STEP1_LEVEL1_OPTIMIZER` | `irls` | Nonlinear Level 1 optimizer: `irls`, `path-newton`, or `newton-cg` |
...
For logistic models, `newton-cg` first applies path-Newton continuation, then
bounded Newton-CG where continuation does not converge, with ordinary IRLS as
the fallback. For Cox models, both `path-newton` and `newton-cg` select the
same safeguarded path continuation; Newton-CG remains logistic-specific. Both
accelerated values are experimental and can change final numerical results.
